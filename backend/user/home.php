...
if(auth($token)){
    ...
    $sql = "SELECT form_id, purchase_form_name, form_status, form_progress, form_image FROM user_subscription_details WHERE user_id=:user_id";
        ...
        if($query->execute()){
            ...
            foreach($not_subscribed_forms as $not_subscribed_form){
                if(!in_array($not_subscribed_form['form_name'], $user_subscriptions)){
                    ...
                    $form_data = [];
                    $form_data['form_id'] = $not_subscribed_form['form_id'];
                    $form_data['purchase_form_name'] = $not_subscribed_form['form_name'];
