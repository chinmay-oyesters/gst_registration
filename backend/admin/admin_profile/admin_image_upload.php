...
if(auth($token)){
    ...
    if(($extn == "jpg") || ($extn == "png") ) {
        ...
        if($_FILES["choosefile"]["size"] < 2000000) {
            ...
            $sql = "UPDATE admin_user_table SET admin_profileimage = :admin_profileimage WHERE admin_user_id = :admin_user_id";
            $query = $con -> prepare($sql);
            $query->bindParam(':admin_user_id', $payload->admin_user_id, PDO::PARAM_STR);
            $query->bindParam(':admin_profileimage', $image, PDO::PARAM_LOB);
            if($query->execute()){
                $status = 200;
                $response = [
